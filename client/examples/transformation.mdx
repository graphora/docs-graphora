...
import os
import time
from graphora import GraphoraClient, TransformRunStatus
...
while not completed:
    ...
    print(
        f"{status.current_stage} | {status.overall_status} | "
        f"{status.percentage_complete:.1f}%"
    )
    ...
    if status.overall_status in (
        ...
        completed = True
        if status.overall_status == TransformRunStatus.COMPLETED:
            print("Transformation completed successfully!")
        else:
            failure = status.error_summary.error_message if status.error_summary else "Unknown error"
            print(f"Transformation failed: {failure}")
    else:
...
try:
    ...
    print(f"Transformation completed with status: {final_status.overall_status}")
