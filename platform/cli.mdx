...
| Office | `.docx`, `.xlsx`, `.pptx` | Converted to markdown via [MarkItDown](https://github.com/microsoft/markitdown) — tables / cells / slide text preserved |
| Text | `.txt`, `.md`, `.csv`, `.json`, `.xml`, `.html` | Ingested directly without conversion |

The parser sniffs by file extension. Directories passed to `graphora extract` are recursed; files with unsupported extensions are skipped silently.
...
See [API Reference — Extraction modes](/api-reference/introduction#extraction-modes) for the full posture comparison and when to pick each.
