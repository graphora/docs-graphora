---
title: 'User Guide'
description: 'Day-to-day GraphCheck: projects, profiles, checks, runs, reports, and baselines'
---

This guide preserves the detailed operational documentation for GraphCheck. For the project
overview and shortest path to a first run, start with the [README](https://github.com/graphora/graphcheck/blob/development/README.md).
...
> **Status:** v0.2.x is released and under active development. The CLI, Neo4j connector, core
...
Cypher 5 sampling path are documented in the [compatibility matrix](/graphcheck/compatibility).
...
[agent guide](/graphcheck/agents) for the MCP tools and agent workflow.
...
a user explicitly opts in. Persistent consent uses `graphcheck telemetry enable` and is revoked
...
behavior. See [the telemetry disclosure](/graphcheck/telemetry) for the complete event and field
inventory.
...
[GraphCheck agent guide](/graphcheck/agents).
...
[`tests/performance`](https://github.com/graphora/graphcheck/tree/development/tests/performance).

See [CONTRIBUTING.md](https://github.com/graphora/graphcheck/blob/development/CONTRIBUTING.md) for the branch workflow, definition of done, and decision
rights.

## License

Apache-2.0. See [LICENSE](https://github.com/graphora/graphcheck/blob/development/LICENSE).
