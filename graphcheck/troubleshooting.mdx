...
description: 'Stable error codes, each with its fix'
---
...
GraphCheck refuses to run anything Neo4j classifies as write-capable, and on Enterprise and Developer
editions it also checks the configured user's actual role before any checks run. The required setup is Neo4j's
...
| `neo4j.credential_read_only_unverified` | Neo4j didn't return the user's roles for inspection | Use Neo4j Enterprise or Developer edition with a user assigned only the built-in `reader` role (plus `PUBLIC`), then run `graphcheck debug` again |
...
- See [CI setup](/graphcheck/ci-setup) for how these map onto exit codes in a pipeline, and
  [Check reference](/graphcheck/check-reference) for check-specific `catches`/`does not catch` behavior.
