---
title: 'Agent Guide'
description: 'How AI agents discover, run, and consume GraphCheck through MCP'
---

This guide describes how an agent discovers and runs GraphCheck suites, consumes the
machine-readable result, and proposes new checks for human approval. It covers the complete
operational contract: the MCP tools, the `results.json` shape an agent reads, and the check
...
| `run_suite` | `suite` (required), `profile` (optional) | Runs one suite with the same read-only credential verification as the CLI, writes the normal run artifacts, and returns a result validated against the `results.json` contract. |
...
check YAML files to the `checks` path configured in `graphcheck.yml` (default: `checks/`) through its
...
Non-Python consumers should validate the structure and then enforce the rules a structural
schema cannot express — the status, totals, score, field-presence, and exit-code relationships
described in the sections of this guide.
...
[Check Reference](/graphcheck/check-reference) lists every built-in conformance check with
